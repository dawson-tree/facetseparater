...
plot_facets_3d <- function(results_obj,
  ...
  for (facet in results_obj$results_list) {
    ...
    # choose rounding digits consistent with raster resolution;
    # default 3 decimals
    round_digits <- 3
    pts_all$key <- paste0(
      round(pts_all$x, round_digits),
      "_", round(pts_all$y, round_digits), "_",
      round(pts_all$z, round_digits)
    )
    inliers_df$key <- paste0(
      round(inliers_df$x, round_digits),
      "_", round(inliers_df$y, round_digits), "_",
      round(inliers_df$z, round_digits)
    )

    # non-inliers equal pts_all minus inliers
    ...
    if (nrow(non_inliers) == 0) {
      pts_all$key_xy <- paste0(
        round(pts_all$x, round_digits), "_",
        round(pts_all$y, round_digits)
      )
      inliers_df$key_xy <- paste0(
        round(inliers_df$x, round_digits), "_",
        round(inliers_df$y, round_digits)
      )
    ...
    grid$z <- plane_coeff["(Intercept)"] + plane_coeff["x"] * grid$x +
      plane_coeff["y"] * grid$y

    # convert to matrix for surface3d
    # (matrix expects byrow = FALSE; dim must match)
    ...
    if (nrow(non_inliers) > 0) {
      rgl::points3d(non_inliers$x, non_inliers$y, non_inliers$z,
        col = "gray70", size = 3
      )
    }
    # plot inliers
    rgl::points3d(inliers_df$x, inliers_df$y, inliers_df$z,
      col = "red", size = 6
    )
    # plane
    rgl::surface3d(
      x = x_seq, y = y_seq, z = z_mat, color = "skyblue",
      alpha = 0.5, front = "fill"
    )
